...
//	social Decimal(2,0),
//	status INTEGER)`)
func BuildTeams(project BuildProject) ([]Employee, error) {
	...
	for i := 0; i < len(project.AOIs); i++ {
		people, err := GetEmployeeArbitrary(fmt.Sprintf("WHERE aoi LIKE %%s% AND status == 0 AND speciality LIKE %%s%", project.AOIs[0].Aoi, project.Type))
		if err != nil {
